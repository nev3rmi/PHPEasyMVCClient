...
(function($) {
    ...
    KEditor.components['text'] = {
        options: {
            ...
            extraPlugins: 'sourcedialog,widget,codesnippet,ajax,xml',
            removePlugins: 'tableselection,tabletools,table,magicline',
...
/**
 * KEditor PHPEASY Carousel Component
 * @copyright: PHPEasy (http://codeasy.cf)
 * @author: NeV3RmI (http://codeasy.cf)
 * @version: 1.0.0
 * @dependencies: $, $.fn.draggable, $.fn.droppable, $.fn.sortable, Bootstrap, FontAwesome (optional)
 */
(function($) {
    ...
    KEditor.components['carousel'] = {
        init: function(contentArea, container, component, keditor) {
            ...
            var componentContent = component.children('.keditor-component-content');
            // componentContent.find('.carousel-wrapper').remove();
        ...
        initSettingForm: function(form, keditor) {
            flog('initSettingForm "carousel" component');

            form.append(
                '<form class="form-horizontal">' +
                '   <div class="form-group">' +
                '       <label for="carousel-id">Carousel Id:</label>' +
                '       <input type="text" class="form-control" id="carousel-id">' +
                '   </div>' +
                '   <div class="form-group">' +
                '       <div class="col-sm-12">' +
                '           <button type="button" class="btn btn-block btn-primary btn-slide-edit">Add Slide</button>' +
                '       </div>' +
                '   </div>' +
                '   <div class="form-group">' +
                '       <label class="col-sm-12"><b>Edit Slide:</b></label>' +
                '       <span class="col-sm-12" id="append-slide">' +
                '       </span>' +
                '   </div>' +
                '   <div class="form-group">' +
                '       <div class="col-sm-12">' +
                '           <button type="button" class="btn btn-block btn-primary btn-slide-submit">Submit</button>' +
                '       </div>' +
                '   </div>' +
                '</form>'
            );

            function _appendSlide(url = '') {
                form.find('#append-slide').append(
                    '<div class="input-group">' +
                    '   <input type="text" class="form-control append-slide-data" placeholder="Image Url" value="' + url + '">' +
                    '   <div class="input-group-btn">' +
                    '       <button class="btn btn-default remove-append-slide-data" type="button"><i class="glyphicon glyphicon-trash"></i></button>' +
                    '   </div>' +
                    '</div>'
                );

                $('.remove-append-slide-data').on('click', function(e) {
                    var self = $(this);
                    var input = self.parent().parent();
                    input.remove();
                })
            }

            // INIT Show url in form
            var imgKeditor = $('.img-carousel-keditor')
            imgKeditor.each(function() {
                var src = $(this).attr('src');
                _appendSlide(src);
            });

            var btnEditSlide = form.find('.btn-slide-edit');
            btnEditSlide.on('click', function(e) {
                e.preventDefault();
                _appendSlide();
            });

            var btnSubmitSlide = form.find('.btn-slide-submit');
            var urlRegex = new RegExp(/[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/);
            btnSubmitSlide.on('click', function(e) {
                e.preventDefault();

                var dataArray = new Array();
                var data = $('.append-slide-data');
                data.each(function() {
                    var value = $(this).val();
                    // Regex URL
                ...
                // Clean
                keditor.getSettingComponent().find('.carousel-indicators').html('');
                keditor.getSettingComponent().find('.carousel-inner').html('');
                ...
                for (i = 0; i < dataArray.length; i++) {
                    ...
                    if (i === 0) {
                        // Indicators
                        keditor.getSettingComponent().find('.carousel-indicators').append(
                            '<li data-target="#keditor-carousel" data-slide-to="0" class="active"></li>'
                        );
                        // Inners
                        keditor.getSettingComponent().find('.carousel-inner').append(
                            '<div class="item active">' +
                            '   <img class="img-responsive img-carousel-keditor" src="' + dataArray[i] + '" alt="image">' +
                            '</div>'
                        );
                    } else {
                        // Indicators
                        keditor.getSettingComponent().find('.carousel-indicators').append(
                            '<li data-target="#keditor-carousel" data-slide-to="' + i + '"></li>'
                        );
                        // Inners
                        keditor.getSettingComponent().find('.carousel-inner').append(
                            '<div class="item">' +
...
/**
 * KEditor PHPEASY Carousel Component V2
 * @copyright: PHPEasy (http://codeasy.cf)
 * @author: NeV3RmI (http://codeasy.cf)
 * @version: 2.0.0
 * @dependencies: $, $.fn.draggable, $.fn.droppable, $.fn.sortable, Bootstrap, FontAwesome (optional)
 */
(function($) { // TODO: Multiple will cause bug. Work only with 1 carousel
    var KEditor = $.keditor;
    var flog = KEditor.log;
    ...
    // Text component
    // ---------------------------------------------------------------------
    KEditor.components['carousel-text'] = {
        // Store data
        stores: {
            prefixId: '#bs-carousel-',
            prefixIdOnly: 'bs-carousel-',
            slideId: '',
            thisSlide: '',
            thisSlideArray: new Array(),
            inputSlideIdVal: '',
            regex: {
                url: new RegExp(/[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/)
            },
            fn: {
                return: function(form, url = '', checked = false) {
                    form.find('#append-slide').append(
                        ...
                        '       <input type="text" class="form-control append-slide-data" placeholder="Image Url" value="' + url + '">' +
                        '       <div class="input-group-btn">' +
                        '           <button class="btn btn-danger remove-append-slide-data" type="button"><i class="glyphicon glyphicon-trash"></i></button>' +
                        '       </div>' +
                        '   </div>' +
                        '   <div class="checkbox">' +
                        '       <label><input type="checkbox" value="1" class="append-slide-data-overlay" ' + (checked ? 'checked' : '') + '>Overlay</label>' +
                        '   </div>' +
                        '</div>'
                    );
                    ...
                    $('.remove-append-slide-data').on('click', function(e) {
                        var self = $(this);
                        var input = self.parent().parent().parent();
                        input.remove();
                    })
                },
                clean: function(form) {
        ...
        init: function(contentArea, container, component, keditor) {
            flog('init "carousel-text" component', component);
        ...
        initSettingForm: function(form, keditor) {
            flog('initSettingForm "carousel-text" component');
        },
        ...
        showSettingForm: function(form, component, keditor) {
            flog('showSettingForm "carousel-text" component');
            // Init
            KEditor.components['carousel-text'].stores.thisSlide = keditor.getSettingComponent();
            KEditor.components['carousel-text'].stores.slideId = keditor.getSettingComponent().find('.carousel').attr("id");
            flog(KEditor.components['carousel-text'].stores.slideId);
            // Clean form
            KEditor.components['carousel-text'].stores.fn.clean(form);

            // Append
            form.append(
                '<form class="form-horizontal carousel-text-setting">' +
                '   <div class="form-group">' +
                '       <div class="col-sm-12">' +
                '           <label for="input-slide-id"><b>Id:</b></label>' +
                '           <div class="input-group">' +
                '               <span class="input-group-addon">' + KEditor.components['carousel-text'].stores.prefixId + '</span>' +
                '               <input type="text" class="form-control" id="input-slide-id" placeholder="Element ID" value="' + KEditor.components['carousel-text'].stores.slideId.replace(KEditor.components['carousel-text'].stores.prefixIdOnly, "") + '">' +
                '           </div>' +
                '       </div>' +
                '   </div>' +
                '   <hr>' +
                '   <div class="form-group">' +
                '       <label class="col-sm-12"><b>Slide:</b> <span><button type="button" class="btn btn-xs btn-success btn-slide-edit"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button></span></label>' +
                '       <span class="col-sm-12" id="append-slide">' +
                '       </span>' +
                '   </div>' +
                '   <hr>' +
            ...
            // Show form
            var slides = keditor.getSettingComponent().find('.slides');
            slides.each(function() {
                var imgUrl = $(this).find('.slide-img').css('background-image').replace('url(', '').replace(')', '').replace(/\"/gi, "");
                var overlayBool = $(this).find('.overlay').length === 1 ? true : false;
                KEditor.components['carousel-text'].stores.fn.return(form, imgUrl, overlayBool);
            });

            // Custom Fn
            function _appendSlide(url = '', checked = false) {
                KEditor.components['carousel-text'].stores.fn.return(form, url, checked);
            };

            function _cleanSlide() {
                keditor.getSettingComponent().find('.carousel-indicators').html('');
                keditor.getSettingComponent().find('.carousel-inner').html('');
                keditor.getSettingComponent().find('.carousel-control').remove();
                KEditor.components['carousel-text'].stores.thisSlideArray = new Array();
            };

            function _getData() {
                // Get Data
                var slideData = form.find('.slide-data');
                // Current element
                var currentSlideData = '';
                var currentSlideOverlay = '';
                // Current element value
                var currentSlideDataValue = '';
                var currentSlideOverlayValue = '';
                // Each slide data
                slideData.each(function() {
                    currentSlideData = $(this).find('.append-slide-data');
                    currentSlideOverlay = $(this).find('.append-slide-data-overlay');

                    currentSlideDataValue = currentSlideData.val();
                    currentSlideOverlayValue = currentSlideOverlay.is(":checked") ? 1 : 0;

                    // Validate
                    if (currentSlideDataValue.match(KEditor.components['carousel-text'].stores.regex.url)) {
                        // Put to Array for further use
                        KEditor.components['carousel-text'].stores.thisSlideArray.push(new Array(currentSlideDataValue, currentSlideOverlayValue));
                    } else {
                        alert(currentSlideDataValue + ' is not URL!');
                        return false;
                    }
                });
            };

            function _changeSlideId() {
                var inputSlide = form.find("#input-slide-id");
                KEditor.components['carousel-text'].stores.inputSlideIdVal = inputSlide.val();
                if (($(document).find(KEditor.components['carousel-text'].stores.prefixId + inputSlide.val()).length == 0)) {
                    keditor.getSettingComponent().find('.carousel').attr("id", KEditor.components['carousel-text'].stores.prefixIdOnly + KEditor.components['carousel-text'].stores.inputSlideIdVal);
                } else if ((KEditor.components['carousel-text'].stores.prefixIdOnly + inputSlide.val()) !== KEditor.components['carousel-text'].stores.slideId) {
                    alert("This Id is already exist, please choose another id");
                    inputSlide.val('');
                    return false;
                }
            };

            function _createLeftRightButton() {
                // Left Button
                KEditor.components['carousel-text'].stores.thisSlide.find('.left-right-move').append(
                    '<a class="left carousel-control" href="' + KEditor.components['carousel-text'].stores.prefixId + KEditor.components['carousel-text'].stores.inputSlideIdVal + '" data-slide="prev">' +
                ...
                // Right Button
                KEditor.components['carousel-text'].stores.thisSlide.find('.left-right-move').append(
                    '<a class="right carousel-control" href="' + KEditor.components['carousel-text'].stores.prefixId + KEditor.components['carousel-text'].stores.inputSlideIdVal + '" data-slide="next">' +
            ...
            function _renderSlide() {
                for (i = 0; i < KEditor.components['carousel-text'].stores.thisSlideArray.length; i++) {
                    if (i === 0) {
                        KEditor.components['carousel-text'].stores.thisSlide.find('.carousel-indicators').append(
                            '<li data-target="' + KEditor.components['carousel-text'].stores.prefixId + KEditor.components['carousel-text'].stores.inputSlideIdVal + '" data-slide-to="' + i + '" class="active">' +
                            '   <div class="thumbnail">' +
                            '       <img class="img-responsive" src="' + KEditor.components['carousel-text'].stores.thisSlideArray[i][0] + '">' +
                            '   </div>' +
                            '</li>'
                        );
                        KEditor.components['carousel-text'].stores.thisSlide.find('.carousel-inner').append(
                            '<div class="item slides active">' +
                            (KEditor.components['carousel-text'].stores.thisSlideArray[i][1] === 1 ? '<div class="overlay"></div>' : '') +
                            '<div class="slide-img" style="background-image: url(\'' + KEditor.components['carousel-text'].stores.thisSlideArray[i][0] + '\');"></div>' +
                            '<div class="hero">' +
                            '<hgroup>' +
                            '<h1>TITLE</h1>' +
                            '<h3>Caption</h3>' +
                            '</hgroup>' +
                            '<a href="javascript:void(0)" class="btn btn-hero btn-lg">Btn Feature</a>' +
                            '</div>' +
                            '</div>'
                        );
                    } else {
                        KEditor.components['carousel-text'].stores.thisSlide.find('.carousel-indicators').append(
                            '<li data-target="' + KEditor.components['carousel-text'].stores.prefixId + KEditor.components['carousel-text'].stores.inputSlideIdVal + '" data-slide-to="' + i + '">' +
                            '   <div class="thumbnail">' +
                            '       <img class="img-responsive" src="' + KEditor.components['carousel-text'].stores.thisSlideArray[i][0] + '">' +
                            '   </div>' +
                            '</li>'
            ...
            // Add Slide
            var BtnAddSlide = form.find(".btn-slide-edit");
            BtnAddSlide.on('click', function() {
                _appendSlide();
            });

            // Submit
            var BtnSubmit = form.find(".btn-slide-submit");
            BtnSubmit.on("click", function() {
                // Clean
                _cleanSlide();

                // Change Id Of Carousel
                _changeSlideId();
...
(function($) {
    ...
    KEditor.components['iframe'] = {
        getContent: function(component, keditor) {
            flog('getContent "iframe" component', component);
        ...
        settingTitle: 'Iframe Settings',

        initSettingForm: function(form, keditor) {
            flog('initSettingForm "iframe" component');

            form.append(
                '<form class="form-horizontal">' +
                '   <div class="form-group">' +
                '       <div class="col-sm-12">' +
                '           <button type="button" class="btn btn-block btn-primary btn-iframe-edit">Update Link</button>' +
                '       </div>' +
                '   </div>' +
                '   <div class="form-group">' +
                '       <label class="col-sm-12">Aspect Ratio</label>' +
                '       <div class="col-sm-12">' +
                '           <button type="button" class="btn btn-sm btn-default btn-iframe-169">16:9</button>' +
                '           <button type="button" class="btn btn-sm btn-default btn-iframe-43">4:3</button>' +
                '       </div>' +
                '   </div>' +
                '</form>'
            );

            var btnEdit = form.find('.btn-iframe-edit');
            btnEdit.on('click', function(e) {
                e.preventDefault();

                var src = prompt('Please enter LINK in here:');
                if (src.length > 0) {
                    keditor.getSettingComponent().find('.embed-responsive-item').attr('src', src);
                } else {
            ...
            var btn169 = form.find('.btn-iframe-169');
            btn169.on('click', function(e) {
                e.preventDefault();
            ...
            var btn43 = form.find('.btn-iframe-43');
            btn43.on('click', function(e) {
                e.preventDefault();
...
(function($) {
    ...
    KEditor.components['view360'] = {
        getContent: function(component, keditor) {
            flog('getContent "view360" component', component);
        ...
        initSettingForm: function(form, keditor) {
            flog('initSettingForm "view360" component');

            form.append(
                '<form class="form-horizontal">' +
                '   <div class="form-group">' +
                '       <div class="col-sm-12">' +
                '           <button type="button" class="btn btn-block btn-primary btn-view360-edit">Update Link</button>' +
                '       </div>' +
                '   </div>' +
                '   <div class="form-group">' +
                '       <label class="col-sm-12">Aspect Ratio</label>' +
                '       <div class="col-sm-12">' +
                '           <button type="button" class="btn btn-sm btn-default btn-view360-169">16:9</button>' +
                '           <button type="button" class="btn btn-sm btn-default btn-view360-43">4:3</button>' +
                '       </div>' +
                '   </div>' +
                '</form>'
            );

            var btnEdit = form.find('.btn-view360-edit');
            btnEdit.on('click', function(e) {
                e.preventDefault();

                var src = prompt('Please enter LINK Image in here:');
                src = 'http://storage.googleapis.com/vrview/index.html?image=' + src + '&%20is_stereo=true';
                if (src.length > 0) {
                    keditor.getSettingComponent().find('.embed-responsive-item').attr('src', src);
                } else {
                    alert('Your LINK Image is invalid!');
                }
            });

            var btn169 = form.find('.btn-view360-169');
            btn169.on('click', function(e) {
                e.preventDefault();
            ...
            var btn43 = form.find('.btn-view360-43');
            btn43.on('click', function(e) {
                e.preventDefault();
...
// TODO: BUGS IN CODE MIRROR
/**
 * CKEditor Full Component
 * @copyright: PHPEasy (http://codeasy.cf)
 * @author: NeV3RmI (http://codeasy.cf)
 * @version: 1.0.0
 * @dependencies: $, $.fn.draggable, $.fn.droppable, $.fn.sortable, Bootstrap, FontAwesome (optional)
...
(function($) {
    ...
    KEditor.components['ckeditor'] = {
        options: {
            ...
            forceEnterMode: true,
            format_tags: 'p;h1;h2;h3;h4;h5;h6',
            extraPlugins: 'sourcedialog,widget,codesnippet,ajax,xml,codemirror',
            // removePlugins: 'tableselection,tabletools,table,magicline',
            // removeButtons: 'Save,NewPage,Preview,Print,Templates,PasteText,PasteFromWord,Find,Replace,SelectAll,Scayt,Form,HiddenField,ImageButton,Button,Select,Textarea,TextField,Radio,Checkbox,Outdent,Indent,Blockquote,CreateDiv,Language,Table,HorizontalRule,Smiley,SpecialChar,PageBreak,Iframe,Styles,BGColor,Maximize,About,ShowBlocks,BidiLtr,BidiRtl,Flash,Image,Subscript,Superscript,Anchor',
            removeButtons: 'Save,NewPage,Scayt,Language,PasteFromWord,PasteText',
            minimumChangeMilliseconds: 100
        },
...
(function($) {
    ...
    // Text component
    // ---------------------------------------------------------------------
    KEditor.components['ckeditor-multieditors'] = {
        options: {
            toolbarGroups: [
                { name: 'document', groups: ['mode', 'document', 'doctools'] }
            ],
...
//     var KEditor = $.keditor;
//     var flog = KEditor.log;
//     var JqueryEditorDialog = new BootstrapDialog({
//         title: 'Jquery Editor',
//         buttons: [{
//             label: 'Save',
...
//     KEditor.components['jquery-editor'] = {
//         getContent: function(component, keditor) {
//             flog('getContent "jquery-editor" component', component);
...
//         settingTitle: 'Jquery Editor Settings',

//         initSettingForm: function(form, keditor) {
//             flog('initSettingForm "jquery-editor" component');
//         },

//         showSettingForm: function(form, component, keditor) {
//             var jqueryValue = component.find("#jquery-editor").text().replace("<br>", "\n").replace("<br />", "\n");
//             console.log(jqueryValue);
//             JqueryEditorDialog.setMessage(
//                 '<div class="form-group">' +
//                 '    <label for="jquery">Jquery:</label>' +
//                 '    <textarea class="form-control" rows="5" id="jquery">' +
//                             jqueryValue +
//                 '    </textarea>' +
//                 '</div>'
//             );
//             JqueryEditorDialog.open();
//         },
//         hideSettingForm: function (form, keditor) {
//             // JqueryEditorDialog.close();
//         }
//     };
