    ...
    $.post('/install/GetCurrentStep', {}, function(result) {
        ...
        form.children("div").steps({
            ...
            onStepChanging: function(event, currentIndex, newIndex) {
                ...
                // Slide 3 - Site info
                if (currentIndex === 3 && resultSummary[3] == null) {
                    var data = new Array();
                    $(".siteinfo-form").each(function() {
                        data.push($(this).val())
                    });
                    var loadingDialog = PHPEasy.Plugins.Custom.Bootstrap.Dialog.Alert.Loading("Request to install site meta, please wait!");
                    $.post('/install/SaveContentCode', {
                        siteName: data[0],
                        sitePublisher: data[1],
                        copyrightYear: data[2],
                        fanpage: data[3],
                        siteEmail: data[4],
                        siteContact: data[5],
                        siteAddress: data[6],
                        ...
                        loadingDialog.setMessage("Saving Google Site Verfication Key...");
                        obj = jQuery.parseJSON(result);
                        loadingDialog.close();
                        if (obj.result === 101) {
                            resultSummary[3] = obj.message + ".......Success!";
                            console.log("Save Google Site Verfication Key: " + obj.message);
                            // Go to next step
                            form.children("div").steps("setStep", 4);
                        } else {
                            PHPEasy.Plugins.Custom.Bootstrap.Dialog.Alert.Fail("Error " + obj.result + ": \n" + obj.message);
                            return false;
                        }
                ...
                // Slide 4 - Google Captcha
                if (currentIndex === 4 && resultSummary[4] == null) {
                    var data = new Array();
                    $(".googlecaptcha-form").each(function() {
                        data.push($(this).val())
                    });
                    var loadingDialog = PHPEasy.Plugins.Custom.Bootstrap.Dialog.Alert.Loading("Request to install google captcha, please wait!");
                    $.post('/install/SaveGoogleCaptcha', { siteKey: data[0], secretKey: data[1] }, function(result) {
                        loadingDialog.setMessage("Saving Google Captcha Key...");
                        obj = jQuery.parseJSON(result);
                        loadingDialog.close();
                        if (obj.result === 101) {
                            resultSummary[4] = obj.message + ".......Success!";
                            console.log("Save Google Captcha Key: " + obj.message);
                            // Go to next step
                            form.children("div").steps("setStep", 5);
                        } else {
                            PHPEasy.Plugins.Custom.Bootstrap.Dialog.Alert.Fail("Error " + obj.result + ": \n" + obj.message);
                            return false;
                        }
